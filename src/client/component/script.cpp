...
namespace script
{
	namespace
	{
		...
		void load_script(std::string& name, const std::string& data, const bool is_custom)
		{
			...
			const auto host_path = (utils::nt::library{}.get_folder() / "codr/").generic_string();
		...
		void load_scripts()
		{
			...
			const auto codr_folder = host.get_folder() / "codr";

			const auto load = [&data_folder, &codr_folder](const std::filesystem::path& folder, const bool is_custom)
			{
				load_scripts_folder((data_folder / folder).string(), is_custom);
				load_scripts_folder((codr_folder / folder).string(), is_custom);
			};
